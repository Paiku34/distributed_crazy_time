...
\begin{itemize}
    ...
    \begin{itemize}
        \item POST /api/wallet/place-bet: validates user balances, creates a unique bet\_id, uses pessimistic locking (@Lock(LockModeType.PESSIMISTIC\_WRITE)) on the database to prevent double-spending, and forwards the payload to RabbitMQ.
    \end{itemize}
...
Note for PowerShell users: set the environment variable using \texttt{\$env:ERL\_FLAGS = "-sname game1@localhost -setcookie crazytime"}.\\
