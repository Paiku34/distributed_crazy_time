...
> [!IMPORTANT]
> **Questo documento è stato riscritto sul codice reale al commit `6ef3b73`, e poi aggiornato dopo il retrofit.** **L'implementazione descritta da questo piano è completa, verifiche comprese.** Gli Step 1-13 di [ordine_implementazione.md](ordine_implementazione.md) sono stati **implementati e verificati** su cluster a 3 nodi con broker e gateway attivi: i punti relativi sono marcati [FATTO] qui sotto. La prima stesura era ancorata al codice della sola **Fase 1** (AMQP nativo). Nel frattempo sono state implementate anche la **Fase 2** (`cluster_manager.erl`) e la **Fase 3** (`leader_election.erl`) **seguendo la stesura originale** di `implementation_plan.md`, cioè senza applicare prima le correzioni strutturali che questo piano richiedeva. Parte del lavoro delle Fasi 2-3 va quindi **corretta**, non aggiunta: i punti interessati sono marcati inline con [FIX].

| Componente | Stato |
| :--- | :--- |
| [rabbitmq_manager.erl](erlang-engine/game_engine/src/rabbitmq_manager.erl) | [FATTO] Fase 1 + `refunds_queue` rimossa da `?QUEUES`. `publish/2` lock-free via ETS, `subscribe/2` col PID del consumer, `ack/1`, `reject/2` ([:91-95](erlang-engine/game_engine/src/rabbitmq_manager.erl#L91-L95)), riconnessione automatica |
| [cluster_manager.erl](erlang-engine/game_engine/src/cluster_manager.erl) | [FATTO] Completo. Discovery, `monitor_nodes(true, [{node_type, all}])`, reconnect, **`configured_nodes/0` e `get_participants/0`**, delega a `leader_election:node_down/1`, **bootstrap Mnesia** a due rami, `subscribe(system)`, `force_load_snapshots/0` |
| [leader_election.erl](erlang-engine/game_engine/src/leader_election.erl) | [FATTO] Fase 3 + retrofit. Bully completo, `apply_role/1` che tocca **solo** il wheel, `broadcast_leader/1` con `{set_leader, N}`, **guardia di quorum** in `declare_victory/1` e `node_down/1`, guardia su `election_in_progress` |
| [worker.erl](erlang-engine/game_engine/src/worker.erl) | [FATTO] Completo: consumer su **ogni** nodo, instradamento al leader, **ack differito** con `inflight` + timeout, **partecipante al taglio** (marker e riporto al collector) |
| [wheel_process.erl](erlang-engine/game_engine/src/wheel_process.erl) | [FATTO] **Completo** — iniziatore del taglio, merge delle bet in transito, abort locale, `settled_bet_ids` (R3), `payouts` con `bet_id`, `complete_round` e `mark_result_published` |
| [game_engine.app.src](erlang-engine/game_engine/src/game_engine.app.src) | [FATTO] `amqp_client` e **`mnesia`** fra le `applications`, `snapshot` fra i `registered`, config broker, `peer_nodes` con tutti e 3 i nodi |
| Java gateway | [FATTO] **Completo** — `bet_id` UUID, dispatch sui quattro tipi, `BetRejectionHandler` idempotente, `LedgerListener` con R1/R2 e `round_cancelled`, `PayoutListener` per round e per `bet_id`, `RefundListener` rimosso |

### Divergenze fra questo piano e il codice attuale

| # | Punto del piano (prima stesura) | Stato nel codice | Azione |
|---|---|---|---|
| 1 | Aggiungere `rabbitmq_manager:reject/2` | [FATTO] **già fatto** ([:91-95](erlang-engine/game_engine/src/rabbitmq_manager.erl#L91-L95)) | Prerequisito chiuso. Resta solo da usarla nel percorso giusto (ack differito), non nel ramo standby |
| 2 | Far consumare `bets_queue` a tutti i nodi | [FATTO] **già fatto**: `subscribe` avviene in `init/1` su ogni nodo ([worker.erl](erlang-engine/game_engine/src/worker.erl)) | La topologia competing-consumer esiste già a livello AMQP: il retrofit della Fase 3 è più piccolo del previsto |
| 3 [FATTO] | `apply_role(standby)` non deve disattivare il worker | [FIX] **il contrario**: `apply_role/1` casta `activate`/`deactivate` al worker ([leader_election.erl](erlang-engine/game_engine/src/leader_election.erl)) | Rimuovere i due cast verso `worker` e sostituirli con `{set_leader, Node}` |
| 4 [FATTO] | Il worker inoltra al leader o rifiuta se non c'è leader | [FIX] il worker in standby fa `reject(Tag, true)` su **ogni** delivery ([worker.erl](erlang-engine/game_engine/src/worker.erl)) | È un **loop caldo di requeue**: il messaggio rimbalza fra broker e standby finché non capita sul leader. La condizione diventa `leader =:= undefined` |
| 5 [FATTO] | Nuova chiave `env` `{nodes, [...]}` come denominatore del quorum | [FIX] non serve: `peer_nodes` esiste già e contiene **tutti e 3** i nodi ([app.src:27](erlang-engine/game_engine/src/game_engine.app.src#L27)) | `configured_nodes/0` legge `peer_nodes`. Attenzione: `cluster_manager` filtra se stesso in `init/1` ([cluster_manager.erl](erlang-engine/game_engine/src/cluster_manager.erl)), quindi la funzione deve **ri-aggiungere** `node()` |
| 6 [FATTO] | Guardia di quorum su `nodedown` dentro `leader_election` | [FIX] `nodedown` è rilevato da `cluster_manager` ([cluster_manager.erl](erlang-engine/game_engine/src/cluster_manager.erl)), non da `leader_election` | La guardia si aggancia lì: `cluster_manager` chiama `leader_election:node_down/1`, che demota se il quorum è perso |
| 7 [FATTO] | «Il worker consuma, acka, poi inoltra al wheel» | La finestra di perdita non si è mai aperta: il canale è passato a `cast` **insieme** all'ack differito | L'ack parte solo su `{bet_result, …}`; allo scadere di 15 s senza esito la scommessa torna al broker |
| 8 | Persistenza su Mnesia | [FIX] `mnesia` non è fra le `applications` ([app.src:7-12](erlang-engine/game_engine/src/game_engine.app.src#L7-L12)) | Aggiungerla, altrimenti l'avvio dipende da un `mnesia:start/0` implicito |
| 9 | «Modifiche a `implementation_plan.md` Fasi 2-3» | [FIX] le Fasi 2-3 sono **codice scritto**, non più carta | La §Parte 4 si divide in *correzioni al codice esistente* (A) e *modifiche ancora solo documentali* (B) |
...
**Rimedio: guardia di quorum prima di `declare_victory/1`.** [FATTO] **Implementata** (Step 3 del Blocco A) e verificata: con 1 nodo su 3 il nodo superstite logga `Quorum 1/3 non raggiunto` e resta standby invece di autoeleggersi.
...
- **due punti di controllo, non uno.** `declare_victory/1` copre solo chi *sta diventando* leader; il leader **già in carica** finito nel lato di minoranza non esegue mai `declare_victory` e resterebbe attivo. Serve quindi la stessa guardia sul `nodedown`.
  [FIX] **Rispetto alla prima stesura cambia dove**: `leader_election` non sottoscrive `net_kernel:monitor_nodes/2` — lo fa `cluster_manager` ([cluster_manager.erl](erlang-engine/game_engine/src/cluster_manager.erl)). Anziché duplicare la sottoscrizione, `cluster_manager` notifica l'elezione:
  ```erlang
...
## Parte 4A — Correzioni al codice già scritto (Fasi 2-3) — [FATTO] APPLICATE

> [FIX] Questa sezione **sostituisce** quella che nella prima stesura descriveva modifiche testuali alle Fasi 2-3 di `implementation_plan.md`: quel codice ora esiste, quindi si tratta di modificarlo, non di riscrivere il piano.

### `cluster_manager.erl` — aggiunte

1. [FATTO] **FATTA — `configured_nodes/0`** — lista **statica** dei nodi, denominatore del quorum. Attenzione: `init/1` filtra il proprio nodo da `peer_nodes` ([cluster_manager.erl](erlang-engine/game_engine/src/cluster_manager.erl)), quindi `known_nodes` **non** contiene `node()` e va ri-aggiunto:
   ```erlang
...
### `leader_election.erl` — correzioni — [FATTO] 1-3 applicate
...
### `worker.erl` — correzioni — [FATTO] applicate
...
## Parte 4B — Modifiche a `implementation_plan.md` — [FATTO] APPLICATE
...
| [Fase 3](implementation_plan.md#L1285) | Marcata [FATTO] IMPLEMENTATA con la tabella delle **5 divergenze**; `apply_role/1` che non tocca più il worker; `broadcast_leader/1` con `{set_leader, N}`; `has_quorum/0` applicata nei due punti; worker senza flag `active`, con `leader` e `inflight`, e instradamento al leader di **tutti** i percorsi; `undo_bets` convertita a `cast` |
...
### `erlang-engine/game_engine/src/rabbitmq_manager.erl` — [FATTO] nessuna modifica necessaria
...
> [FATTO] I punti **1-4** e **6-9** sono implementati (Blocchi A e B): campo `leader` al posto di `active`, `{set_leader, N}`, instradamento al leader di tutti i percorsi, canale asincrono con **ack differito**, `inflight_timeout`, `bet_id` nel parser, rimborso puntuale al posto di `refunds_queue`. Resta il punto **5**, gli handler dei marker dello snapshot.

1. [FATTO] **FATTO** — State record: `#state{leader = undefined, cl = cl_recorder:new(), inflight = #{}}`, dove `inflight :: #{BetId => {DeliveryTag, BetMap}}` — serve sia allo snapshot sia all'ack differito. [FIX] **Il campo `active` sparisce** ([worker.erl](erlang-engine/game_engine/src/worker.erl)).
2. [FATTO] **FATTO** — `handle_cast({set_leader, Node})` — memorizza il leader; **sostituisce** `handle_cast(activate|deactivate, ...)` ([worker.erl](erlang-engine/game_engine/src/worker.erl)).
3. [FATTO] **FATTO** — `process_message/3` non fa più `gen_server:call(wheel_process, ...)` ([worker.erl](erlang-engine/game_engine/src/worker.erl)): diventa `gen_server:cast({wheel_process, Leader}, {bet, BetMap})`.
   **L'ack AMQP non è più immediato**: oggi parte al ritorno di `process_message/1` ([worker.erl](erlang-engine/game_engine/src/worker.erl)), che essendo una `call` sincrona locale garantisce «il wheel ha deciso». Passando al `cast` quella garanzia si perderebbe: il `DeliveryTag` va quindi conservato in `inflight` e l'ack parte solo alla ricezione di `{bet_result, BetId, accepted | rejected}`. Motivazione completa in §Parte 2.
   Se `leader = undefined` (nessun leader eletto, o nodo in minoranza dopo una partizione) la bet non viene inoltrata: `rabbitmq_manager:reject(Tag, true)`, così resta nel broker e verrà consegnata a un nodo che può servirla. [FIX] Questo ramo **assorbe e sostituisce** la clausola di delivery in standby ([worker.erl](erlang-engine/game_engine/src/worker.erl)): il requeue avviene solo quando *nessuno* può servire il messaggio, non ogni volta che lo riceve un non-leader.
4. [FATTO] **FATTO** — Instradare al leader anche gli altri quattro percorsi di `process_message/3` ([worker.erl](erlang-engine/game_engine/src/worker.erl)). Il punto 3 converte il solo ramo bet, ma la stessa funzione instrada oggi **verso il `wheel_process` locale**: `type: "force_segment"` ([worker.erl](erlang-engine/game_engine/src/worker.erl)) e `FORCE_<Seg>` ([worker.erl](erlang-engine/game_engine/src/worker.erl)), `type: "minigame_choice"` ([worker.erl](erlang-engine/game_engine/src/worker.erl)), `segment: "UNDO_BETS"` ([worker.erl](erlang-engine/game_engine/src/worker.erl)).
...
6. [FATTO] **FATTO** — `handle_cast({bet_result, BetId, Verdict})` è l'unico punto in cui si acka, poi `rabbitmq_manager:ack(Tag)` e rimuove da `inflight`. È l'unico punto in cui si acka.
7. [FATTO] **FATTO** — **Timeout dell'inflight**: `{inflight_timeout, BetId}` armato a 15 s al momento del cast. Se scade (leader morto prima di rispondere) → `rabbitmq_manager:reject(Tag, true)`: la bet torna nel broker e sarà riconsegnata a un worker vivo. La perdita silenziosa sparisce.
8. [FATTO] **FATTO** — **`publish_refund/1` riscritto, non rimosso** ([worker.erl](erlang-engine/game_engine/src/worker.erl)).
...
9. [FATTO] **FATTO** — `parse_bet_json/1` estrae anche `<<"bet_id">>` con `extract_string_field/2` ([worker.erl](erlang-engine/game_engine/src/worker.erl)). I comandi `force_segment` e `minigame_choice` non ne hanno uno: `bet_id = undefined` deve saltare sia la dedup di R3 sia la mappa `inflight`, e questi messaggi restano ad **ack immediato** — l'ack differito riguarda solo le bet.

### [MODIFY] `erlang-engine/game_engine/src/wheel_process.erl`
1. [FATTO] **FATTO** (tranne `settled_bet_ids`) — promuovere lo stato del round nel record ([wheel_process.erl](erlang-engine/game_engine/src/wheel_process.erl)); oggi segmento vincente, indice e dettagli del minigioco vivono solo dentro i messaggi `send_after` ([wheel_process.erl](erlang-engine/game_engine/src/wheel_process.erl), [wheel_process.erl](erlang-engine/game_engine/src/wheel_process.erl), [wheel_process.erl](erlang-engine/game_engine/src/wheel_process.erl)):
   ```erlang
   -record(state, {
...
2. [FATTO] **FATTO** — `handle_cast({bet, BetMap, FromNode}, S)` sostituisce `handle_call({place_bet, ...})` ([wheel_process.erl](erlang-engine/game_engine/src/wheel_process.erl)).
...
3. [FATTO] **FATTO** — **Trigger del taglio** in `handle_info(tick, #state{phase = betting, time_left = 1})` ([wheel_process.erl](erlang-engine/game_engine/src/wheel_process.erl)) — **prima** estrarre il vincitore ([wheel_process.erl](erlang-engine/game_engine/src/wheel_process.erl)), **poi** iniziare:
   ```erlang
   ...
   ```
4. [FATTO] **FATTO** — `handle_cast({cl_marker, SnapId, {worker, N}}, S)` — chiude il canale `{worker,N}`; se `cl_recorder:is_complete/1`:
...
5. [FATTO] **FATTO** — `handle_info({cl_abort, SnapId}, S)` — chiude forzatamente la registrazione, riporta ciò che ha, logga `degraded`.
6. [FATTO] **FATTO** — `build_result_json/7` ([wheel_process.erl](erlang-engine/game_engine/src/wheel_process.erl)) — `bet_id` aggiunto a ogni entry dell'array `payouts`. **Vale per tutti i produttori di payout**, non solo per `wheel_process:compute_payouts/3`: i minigiochi asincroni `crazytime` e `cashhunt` costruiscono le proprie entry, e senza il loro `bet_id` il gateway non trova corrispondenza, ricade sul campo `multiplier` — che per gli asincroni è la sentinella `-1` — e **salta il pagamento**.
7. [FATTO] **FATTO** — Ogni `{bet_result, BetId, rejected}` è accompagnato da un `bet_rejected` su `results_queue` (formato in §`worker.erl` punto 8). È il solo percorso che chiude le bet arrivate dopo la pubblicazione del ledger; senza, restano `PENDING` a saldo scalato. `publish_to_queue/2` ([wheel_process.erl](erlang-engine/game_engine/src/wheel_process.erl)) è lock-free (ETS + `amqp_channel:cast`), quindi non blocca il wheel.
...
### [NEW] tabella Mnesia `snapshot_record` — [FATTO] CREATA (in `include/game_engine.hrl`)
```erlang
...
### [MODIFY] `game_engine_sup.erl` — [FATTO] FATTO
...
1. [FATTO] **FATTO** — **`entity/Bet.java`** — nuovo campo `@Column(unique = true) String betId`. `ddl-auto=update` ([application.properties:13](java-gateway/src/main/resources/application.properties#L13)) crea la colonna senza migrazione manuale.
2. [FATTO] **FATTO** — **`WalletController`** — generare `UUID.randomUUID().toString()`, persisterlo sulla `Bet` e includerlo nel JSON. Sostituire la concatenazione di stringhe con Jackson (l'username oggi non è escapato).
3. [FATTO] **FATTO** — **`BetRepository.java`** — aggiungere `findByBetId(String)`, `findByRoundAndStatus(Integer, String)`.
4. [FATTO] **FATTO** — **`GameResultListener`** — **dispatch sul campo `type`** (restano i rami `round_ledger` e `round_cancelled`), oggi completamente ignorato: qualunque messaggio su `results_queue` viene trattato come risultato di round. Instradare `result` → `PayoutListener`, `round_ledger` → nuovo `LedgerListener`, `round_cancelled` → cancellazione per-round, `bet_rejected` → rimborso puntuale per `bet_id`.
5. [FATTO] **FATTO** — **[NEW] `rabbitmq/LedgerListener.java`** — `@Transactional`, implementa R1 e R2 della §Parte 3:
...
8. [FATTO] **FATTO** — **`PayoutListener`** — `findByRoundAndStatus(round, "PENDING")` al posto del `findByStatus("PENDING")` globale; match per `bet_id` invece che per `username` ([PayoutListener.java:64](java-gateway/src/main/java/com/crazytime/rabbitmq/PayoutListener.java#L64)); rimuovere `it.remove()`.
9. [FATTO] **FATTO** — **`RefundListener.java`** — classe e coda **rimosse**: la sua funzione è assorbita dall'handler `bet_rejected`, che identifica la bet per `bet_id` invece che per importo (`RefundListener`).
...
10. [FATTO] **FATTO in `PayoutListener`** — rimuovere il `catch` che inghiotte le eccezioni in `PayoutListener:107-109` e `RefundListener:77-79`: essendo i metodi `@Transactional`, l'eccezione catturata non provoca rollback e i `save` parziali vengono committati.
...
| **Partizione di rete 2-1, isolando il LEADER in carica** — [FATTO] **eseguito** | È il caso che `declare_victory` non copre: il vecchio leader deve autoretrocedersi da `leader_election:node_down/1` entro il tempo di rilevazione. Un solo ledger pubblicato. Se il vecchio leader continua a pubblicare, la guardia è nel posto sbagliato |
...
5. [FATTO] **FATTO — UC2 lato Java**: dispatch su `type`, handler `bet_rejected`, `LedgerListener` con le regole R1/R2, `PayoutListener` per-round, rimozione di `RefundListener` e di `refunds_queue` (solo dopo il passo 1).
6. [FATTO] **FATTO — Fase 5 corretta**: recovery dal checkpoint; fallback `round_cancelled` con `exclude_bet_ids`, raccolti via `{collect_inflight, R}`. Il discriminante fra i due rami è il campo `result_published` aggiunto al checkpoint. Il recovery parte **solo** alla transizione standby → leader.
7. [FATTO] **FATTO — Fase 6**: test della Parte 7 percorsi tutti, **partizione di rete inclusa** (partizione logica stabile con `net_kernel:allow/1`, non regole firewall). Il leader isolato si autoretrocede, la maggioranza elegge, esiste un solo `round_ledger` per round e alla ricomposizione compare `inconsistent_database`.
