...
7. ✅ **FATTO — Fase 6**: test della Parte 7 percorsi tutti, **partizione di rete inclusa** (partizione logica stabile con `net_kernel:allow/1`, non regole firewall). Il leader isolato si autoretrocede, la maggioranza elegge, esiste un solo `round_ledger` per round e alla ricomposizione compare `inconsistent_database`.
