...
> [!IMPORTANT]
> **Questo documento è stato riscritto sul codice reale al commit `6ef3b73`, e poi aggiornato dopo il retrofit.** Gli Step 1-3 di [ordine_implementazione.md](ordine_implementazione.md) — Blocco A — sono stati **implementati e verificati su cluster a 3 nodi**: i punti relativi sono marcati ✅ qui sotto. La prima stesura era ancorata al codice della sola **Fase 1** (AMQP nativo). Nel frattempo sono state implementate anche la **Fase 2** (`cluster_manager.erl`) e la **Fase 3** (`leader_election.erl`) **seguendo la stesura originale** di `implementation_plan.md`, cioè senza applicare prima le correzioni strutturali che questo piano richiedeva. Parte del lavoro delle Fasi 2-3 va quindi **corretta**, non aggiunta: i punti interessati sono marcati inline con 🔧.
...
| [worker.erl](erlang-engine/game_engine/src/worker.erl) | ✅ consumer AMQP su **ogni** nodo, campo `leader` al posto del flag `active`, instradamento al leader di **tutti** i percorsi, requeue solo quando non c'è leader. ❌ restano ack differito, `inflight`, `bet_id` |
| [wheel_process.erl](erlang-engine/game_engine/src/wheel_process.erl) | ✅ flag `active`, `activate`/`deactivate`, tick guardati, `undo_bets` come **cast** con rimborso pubblicato dal wheel. ❌ stato del round nel record, dedup, snapshot |
...
| 7 | «Il worker consuma, acka, poi inoltra al wheel» | 🔧 descrizione superata: oggi l'inoltro è una `gen_server:call` **sincrona e locale** e l'ack arriva **dopo** ([worker.erl](erlang-engine/game_engine/src/worker.erl), [worker.erl](erlang-engine/game_engine/src/worker.erl)) | La finestra di perdita **oggi non esiste**: verrebbe *introdotta* dal passaggio a `cast`. L'ack differito la chiude prima che si apra — vedi §Parte 2 |
...
> ✅ I punti **1-4** sono stati implementati dal retrofit (Blocco A): il campo `leader` ha sostituito il flag `active`, `{set_leader, N}` è in funzione, e **tutti** i percorsi di `process_message/3` vanno al wheel del leader. Restano da fare i punti **5-9**, cioè l'ack differito e lo snapshot.
...
3. 🔧 **PARZIALE** — `process_message/3` instrada già al leader, ma la bet passa ancora da una `call` sincrona con timeout (impalcatura temporanea). Da completare: **niente più `gen_server:call(wheel_process, ...)`** ([worker.erl](erlang-engine/game_engine/src/worker.erl)): diventa `gen_server:cast({wheel_process, Leader}, {bet, BetMap})`.
...
8. **`publish_refund/1` va riscritto, non rimosso** ([worker.erl](erlang-engine/game_engine/src/worker.erl)).
...
9. `parse_bet_json/1` ([worker.erl](erlang-engine/game_engine/src/worker.erl)) — estrarre anche `<<"bet_id">>` con `extract_string_field/2` ([worker.erl](erlang-engine/game_engine/src/worker.erl)). I comandi `force_segment` e `minigame_choice` non ne hanno uno: `bet_id = undefined` deve saltare sia la dedup di R3 sia la mappa `inflight`, e questi messaggi restano ad **ack immediato** — l'ack differito riguarda solo le bet.

### [MODIFY] `erlang-engine/game_engine/src/wheel_process.erl`
1. **Prerequisito** — promuovere lo stato del round nel record ([wheel_process.erl](erlang-engine/game_engine/src/wheel_process.erl)); oggi segmento vincente, indice e dettagli del minigioco vivono solo dentro i messaggi `send_after` ([wheel_process.erl](erlang-engine/game_engine/src/wheel_process.erl), [wheel_process.erl](erlang-engine/game_engine/src/wheel_process.erl), [wheel_process.erl](erlang-engine/game_engine/src/wheel_process.erl)):
   ```erlang
   -record(state, {
...
2. `handle_cast({bet, BetMap}, S)` — sostituisce `handle_call({place_bet, ...})` ([wheel_process.erl](erlang-engine/game_engine/src/wheel_process.erl)).
...
3. **Trigger del taglio** in `handle_info(tick, #state{phase = betting, time_left = 1})` ([wheel_process.erl](erlang-engine/game_engine/src/wheel_process.erl)) — **prima** estrarre il vincitore ([wheel_process.erl](erlang-engine/game_engine/src/wheel_process.erl)), **poi** iniziare:
   ```erlang
